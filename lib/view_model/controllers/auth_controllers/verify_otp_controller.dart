    ...
    if(start.value == 0){
      ...
      try {
        await authProvider.resendOtpTap(onError: (errorMessage){
          ...
          toastShow(massage: "Otp has been sent on your email",error: false);
        });
      } catch (e) {
        ...
        try {
          ...
          await authProvider.otpVerifyTap(otpData, onError: (errorMessage){
            ...
            try{
              ...
              await Get.find<AuthService>().saveUserToken(userDataModel.token!.access!.token??"");
              toastShow(massage:"You are login successfully",error: false);
              Get.find<AuthService>().user.value.isKyc!=null && Get.find<AuthService>().user.value.isKyc==true?Get.find<AuthService>().user.value.storeRegistered!=null&&Get.find<AuthService>().user.value.storeRegistered==true?Get.offAllNamed(Routes.rootView):Get.offAllNamed(Routes.storeRegistrationScreen):Get.offAllNamed(Routes.verificationScreen);
              isLoading.value = false;
            }catch(e){
              logPrint(e.toString());
